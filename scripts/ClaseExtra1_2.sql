...
SELECT * FROM tabla1 LIMIT 5;

-- El total de filas de la tabla1
SELECT COUNT(*) FROM tabla1;

-- Paginacion
-- A partir del registro 100, retornamos 5 registros
SELECT * FROM tabla1 LIMIT 7 OFFSET 1000;
...
SELECT sexo, AVG(sueldo) FROM tabla1
GROUP BY sexo;

-- Promedio de sueldo por region
SELECT region, ROUND(AVG(sueldo)) FROM tabla1
GROUP BY region;
...
SELECT * FROM
	(SELECT region, sexo, ROUND(AVG(sueldo)) AS Promedio_Sueldo 
	 FROM tabla1
	 GROUP BY region, sexo) AS tabla_aux
WHERE sexo IN ('O', 'F')
ORDER BY Promedio_Sueldo DESC;	 
	 
-- Suma del sueldo promedio por region por sexo
...
--- UNION : hace alusion a apilar tablas
...
SELECT *
FROM tabla1 NATURAL JOIN tabla3;
